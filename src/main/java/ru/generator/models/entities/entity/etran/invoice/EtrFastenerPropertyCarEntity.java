...
@Data
@Entity
@Table(name = "fastener_property_car", schema = "etran_inv",
        indexes = {
                @Index(columnList = "id_invoice"),
                @Index(columnList = "id_car_fastener"),
                @Index(columnList = "id_inv_car")
        })
public class EtrFastenerPropertyCarEntity {
    @Id
    ...
    @ManyToOne
    @JoinColumn(name = "id_car_fastener", referencedColumnName = "id", nullable = false)
    @IgnoreField
    private EtrCarFastenerEntity carFastener;
    @ManyToOne
